...
class GatedGraphConv(tgnn.conv.MessagePassing):
    ...
    def __init__(self, out_channels: int, num_layers: int, num_edge_types: int = 1,
                 aggr: str = 'add', bias: bool = True, normalize: bool = False, **kwargs):
        super(GatedGraphConv, self).__init__(aggr=aggr, **kwargs)

        self.out_channels = out_channels
        self.num_layers = num_layers
        self.num_edge_types = num_edge_types
        self.weight = nn.Parameter(Tensor(num_edge_types, out_channels, out_channels))
        self.rnn = torch.nn.GRUCell(out_channels, out_channels, bias=bias)
        self.normalize = normalize
        self.norm = tgnn.PairNorm()
        self.relu = nn.ReLU()
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_attr: OptTensor=None,
        ...
        for l in range(self.num_layers):
            ...
            x = self.rnn(m, x)
            if self.normalize and l < self.num_layers -1:
                x = self.relu(x)
                x = self.norm(x)
        return x
    ...
    def message(self, x_j: Tensor, edge_attr: OptTensor=None,
        ...
        if self.weight.size(0) != 1 and edge_attr is not None:
            ...
            for idx in range(self.num_edge_types):
                ...
                edge_indices = edge_attr[:, idx] == 1
                # x_j_edge = torch.zeros((x_j.size()), dtype=x_j.dtype)
                x_j_edge = torch.zeros((x_j.size())).type_as(x_j)
                x_j_edge[edge_indices, :] = x_j[edge_indices, :] @ edge_attr_weight
