...
public abstract class Node implements Cloneable, HasParentNode<Node>, Visitable, NodeWithRange<Node>, NodeWithTokenRange<Node> {
    ...
    private static Comparator<Node> nodeComparator = Comparator.comparingInt(n -> n.hashCode(true));

    private static Map<OtherLinkType, ListOrderedSet<Node>> getMap() {
        Map<OtherLinkType, ListOrderedSet<Node>> map = new HashMap<>();
        for (OtherLinkType other : OtherLinkType.values()) {
            map.put(other, ListOrderedSet.listOrderedSet(new TreeSet<>(nodeComparator)));
        }
        return map;
    }
    ...
    protected static final PrettyPrinterConfiguration prettyPrinterNoCommentsConfiguration = new PrettyPrinterConfiguration()
    ...
    @InternalProperty
    private Map<OtherLinkType, ListOrderedSet<Node>> otherNodes = getMap();
    ...
    public final int hashCode(boolean fromSuper) {
        return 31 * this.getClass().hashCode() + super.hashCode();
    }
    ...
    public void setChildNodes(List<Node> childNode) {
     ...
     * @return all nodes that have this node as their parent.
     */
    public Map<OtherLinkType, ListOrderedSet<Node>> getOthersNodes() {
        return otherNodes;
    }
